...
final class AppleEcosystemBridge: NSObject, WKScriptMessageHandlerWithReply {
    ...
    /// JavaScript code injected into the runtime to provide the `apple.*` API.
    /// Each method posts a message to the native bridge via WKScriptMessageHandlerWithReply
    /// and returns a Promise. Use `await` in callAsyncJavaScript context to get results.
    nonisolated static let jsPreamble: String = """
    var apple = (function() {
        async function call(action, args) {
            try {
                return await window.webkit.messageHandlers.appleBridge.postMessage({action: action, args: args || {}});
            } catch(e) {
