    ...
    func convertWithImageForwarding(_ msgs: [Message]) -> [LLMChatMessage] {
        ...
        for msg in msgs {
            ...
            case .user:
                ...
                if !allImages.isEmpty || deletedCount > 0 {
                    var finalText = text
                    if deletedCount > 0 {
                        finalText += "\n[Note: \(deletedCount) image(s) were deleted by the user and are no longer available.]"
                    }
                    if forwardedCount > 0 {
                        let hint = "[Note: \(forwardedCount) of the attached image(s) were generated by the assistant in a previous response, not sent by the user.]"
                        finalText = hint + "\n\n" + finalText
                    }
                    if allImages.isEmpty {
                        result.append(.user(finalText))
                    } else {
                        result.append(.userWithImages(finalText, images: allImages))
                    }
                } else {
        ...
        if !pendingImages.isEmpty {
            let hint = "[The following \(pendingImages.count) image(s) were generated by the assistant in a previous response.]"
            result.append(.userWithImages(hint, images: pendingImages))
        }
